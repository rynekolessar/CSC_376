...
std::string parseJson(struct MemoryStruct *chunkPtr)
{
	...
	JSONValue *rootPtr = JSONValue::factory(chunkPtr->memory);
	std::string retStr;
	if (rootPtr != NULL)
	{
		JSONPath vPath;
		JSONPath uPath;
		const JSONValue *valuePtr;
		const JSONValue *unitsPtr;
		std::string value;
		std::string units;
		vPath.addStr("properties");
		vPath.addStr("temperature");
		vPath.addStr("values");
		vPath.addInt(0);
		vPath.addStr("value");
		valuePtr = vPath.follow(rootPtr);
		value = (valuePtr == NULL) ? "(unknown)" : valuePtr->getString(false);
		uPath.addStr("properties");
		uPath.addStr("temperature");
		uPath.addStr("uom");
		unitsPtr = uPath.follow(rootPtr);
		units = (unitsPtr == NULL) ? "(unknown)" : unitsPtr->getString(false);
		retStr = value + " " + units.replace(0, 5, ""); //"%s %s\n", value.c_str(), units.c_str());
		delete (rootPtr);
	}
...
protected:
	...
	void endPython()
	{
		...
		//  TODO: Done
		write(toPythonFd_, END_PYTHON_CONST_CPTR, END_PYTHON_LEN);
	}
...
public:
	...
	std::string evalExpression(std::string toEvalStr)
	{
		...
		return (waitForPrompt(fromPythonFd_));
	}
	...
	std::string getExternalContentStr()
	{
		...
		//  TODO: Done
		// set a MemoryStruct variable as the CURLOPT_WRITEDATA for curlHandle_ and do the libcurl call
		chunk.memory = (char *)malloc(1);
		chunk.size = 0;
		// set chunk as CURLOPT_WRITEDATA for curlHandle_
		curlHandle_ = curl_easy_init();
		curl_easy_setopt(curlHandle_, CURLOPT_WRITEDATA, chunk);
		// libcurl call
		res = curl_easy_perform(curlHandle_);
		// if it succeeds put a '/0' at the end of the memory member variable
		if (res == CURLE_OK)
		{
			chunk.memory = chunk.memory + '\0';
		}
		// free the memory
		free(chunk.memory);
		// Return the value returned by parseJSON
							...
							SessionStore &sessionStore)
{
	...
	if (strncmp(filePath, SYM_EVAL_PAGE_NAME, strlen(SYM_EVAL_PAGE_NAME)) == 0)
	{
		...
		//  TODO: Done
		request.getQuery()["ChiTemp"] = content.getExternalContentStr();
	}
